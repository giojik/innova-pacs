import os, json, csv, datetime, psycopg2
...
async def admin_dashboard(period: str = "month"):
    ...
    # HTML სექციების აწყობა
    stats_html = "".join([f"<div class='bg-slate-50 p-4 rounded-2xl border text-center'><b>{r[0]}</b><br><span class='text-xl font-black text-blue-600'>{r[1]}</span></div>" for r in mod_stats])
    
    log_rows = "".join([f"<tr class='border-b'><td class='p-3'>{l[0]}</td><td class='p-3'>{l[1]}</td><td class='p-3'>{l[3]}</td><td class='p-3'>{l[4]}</td><td class='p-3 font-bold text-blue-600'>{l[6]}</td></tr>" for l in recent_logs])
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="space-y-6">
                    <div class="bg-white p-6 rounded-[35px] shadow-sm border-t-8 border-blue-500">
                        ...
                        {"".join([f"<div class='p-2 bg-blue-50 rounded-lg mb-1 text-[11px] font-bold border-l-4 border-blue-400'>{v['name']}: {v['percent']}%</div>" for k,v in live_sync.items()]) if live_sync else '<p class="text-slate-300 italic text-xs">რიგი ცარიელია</p>'}
                    </div>
                    <div class="bg-white p-6 rounded-[35px] shadow-sm border-t-8 border-orange-500">
                        ...
                        {"".join([f"<div class='p-2 bg-orange-50 rounded-lg mb-1 text-[11px] font-bold border-l-4 border-orange-400'>{v['name']}: {v['percent']}%</div>" for k,v in migration.items()]) if migration else '<p class="text-slate-300 italic text-xs">რობოტი ისვენებს</p>'}
                    </div>
                </div>
